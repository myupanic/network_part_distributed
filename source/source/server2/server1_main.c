...
void manager(int sig) {
	pid_t pid_chld;
	//printf("Ho ricevuto il segnale SIGCHLD\n");
	pid_chld = wait(NULL);
	//printf("Il processo figlio %d e' terminato\n", pid_chld);
}
...
int main (int argc, char *argv[])
{
	...
	// ignoro segnali SIGCHLD provenienti dai processi figli
	...
	while(1) { // in questo ciclo (senza considerare quello ancora piu' interno) si esegue la accept() ovvero si accetta la connessione
		...
		//printf("il socket s vale: %d con pid %d\n", s, getpid());
		...
		pid = fork();
		if( pid < 0 ) {
			printf("Fork failed. Killing server process.\n");
			exit(-1);
		}
			...
			while(1) { // in questo ciclo viene eseguito tutto cio' che serve per ricevere la richiesta del client ed inviare il file richiesto
				...
				//printf("Figlio in azione per inviare nuovo file con pid %d\n", getpid());
				close(s);
				...
				if( receivingMsgSize == 0 ) { // non c'e' niente da leggere e il client ha chiuso la connessione: mi metto in attesa di una 
					...
					//printf("Child process having PID %d terminates.\n", getpid());
					//close(sock);
					exit(0);
				}
